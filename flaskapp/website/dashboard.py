...
def add_config():
    if request.method == 'POST':
        ...
        try:
            ...
            authorization_name = data.get('authorization_name', '').strip()
            if authorization_name:
                connector = HTTPConnector.query.filter_by(name=authorization_name).first()
                if connector and connector.auth_type != 'none':
                    ...
                    elif connector.auth_type == 'api_key' and not auth_config.get('in'):
...
def edit_config(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            config.headers = json.dumps(headers)
            ...
            config.authorization_name = authorization_name
            db.session.commit()
...
#             if auth_header:
#                 f.write(f"    headers = {json.dumps(auth_header, indent=4)}\n")
...
def generate_tool():
    ...
    with open('tool.py', 'w') as f:
        ...
        for config in configs:
            ...
            if config.authorization_name:
                ...
                if connector and connector.auth_type != 'none':
                    ...
                    if connector.auth_type == 'basic':
                        ...
                        key_value = auth_config.get('key_value', '')
                        if key_name and key_value:
                            if auth_config.get('in') == 'query':
                                # Add to function parameters if not already present
                                if key_name not in param_dict:
                                    param_args.append(f'{key_name}: str = "{key_value}"')
                                query_params.append(f'"{key_name}": {key_name}')
                            else:
                                auth_header[key_name] = key_value
            ...
            f.write(f"            url,\n")
            f.write(f"            headers=headers,\n")
            if config.method.upper() in ['GET', 'DELETE'] or query_params:
                f.write("            params=params,\n")
            if config.method.upper() in ['POST', 'PUT']:
                f.write("            json=json_data,\n")
            f.write("        )\n")
